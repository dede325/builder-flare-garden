...
import { intelligentSyncService, SyncStats, ConnectionStatus } from '@/lib/intelligent-sync-service';
...
export default function SyncStatusIndicator({ compact = false, showManualSync = true }: SyncStatusIndicatorProps) {
    ...
    return (
      <div className="flex items-center space-x-2">
        ...
        <div className="flex items-center space-x-1">
          ...
          />
          <Badge className={`${getSyncStatusColor()} text-white text-xs px-2 py-0`}>
            {getSyncStatusText()}
          </Badge>
        </div>

        {/* Manual Sync Button */}
        {showManualSync && connectionStatus.isOnline && syncStats.pendingItems > 0 && (
          <Button
            variant="ghost"
            size="sm"
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            {connectionStatus.connectionType && (
              <span className="text-white/70 text-xs">
                {connectionStatus.effectiveType} • {connectionStatus.connectionType}
              </span>
            )}
          </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-red-300 text-sm">
                    {syncStats.errorItems} item{syncStats.errorItems !== 1 ? 's' : ''} com erro
                  </span>
                </div>
                <Button
                  variant="outline"
          ...
          {syncStats.lastSync && (
            <div className="text-xs text-white/50">
              Última sincronização: {format(new Date(syncStats.lastSync), "dd/MM/yyyy HH:mm", { locale: ptBR })}
            </div>
          )}
          ...
          <div className="flex items-center space-x-2">
            {showManualSync && connectionStatus.isOnline && (
              ...
              >
                <RefreshCw className={`h-4 w-4 mr-2 ${isSyncing || syncStats.syncInProgress ? 'animate-spin' : ''}`} />
                {isSyncing || syncStats.syncInProgress ? 'Sincronizando...' : 'Sincronizar Agora'}
              </Button>
            )}
              ...
              <DialogContent className="max-w-2xl bg-aviation-gray-800 border-white/20">
                <DialogHeader>
                  ...
                  <DialogDescription className="text-white/70">
                    Informações detalhadas sobre o status de sincronização dos dados
                  </DialogDescription>
                </DialogHeader>
          ...
          syncLogs.map((log, index) => {
            ...
            return (
              <div key={index} className="flex items-start space-x-3 p-3 rounded-lg bg-white/5 border border-white/10">
                <LogIcon className={`h-4 w-4 mt-0.5 ${getLogColor(log.type)}`} />
