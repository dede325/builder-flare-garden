...
class CodeGenerationService {
  ...
  generateFormCode(location?: string, shift?: string): string {
    ...
    if (location && shift) {
      ...
      sequenceNumber = parseInt(`${locationCode}${shiftCode}${String(this.sequenceCounter).padStart(2, '0')}`);
    }
   ...
   */
  private async generateQRCodeUrl(formCode: string, formId: string): Promise<string> {
    try {
   ...
   */
  async generateQRCodeSet(formCode: string, formId: string): Promise<{
   ...
   */
  generateBatchCodes(count: number, location?: string, shift?: string): string[] {
    const codes: string[] = [];
   ...
   */
  async storeQRCodeMetadata(formCode: string, formId: string, qrCodeData: string): Promise<boolean> {
    try {
   ...
   */
  async generateUniqueFormCode(location?: string, shift?: string): Promise<string> {
    let attempts = 0;
  ...
  generateCustomFormCode(params: {
    ...
    if (params.location && params.shift) {
      ...
      sequence = parseInt(`${locationCode}${shiftCode}${String(sequence).padStart(2, '0')}`);
    }
