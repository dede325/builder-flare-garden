...
if (fs.existsSync(migrationDir)) {
  ...
  const vfinalMigrations = migrations.filter(m => m.includes('vfinal') || m.includes('20241220'));
  if (vfinalMigrations.length < 4) {
    errors.push('❌ Migrações VFINAL incompletas. Esperadas: 4, encontradas: ' + vfinalMigrations.length);
  } else {
...
if (fs.existsSync(migrationsFile)) {
  ...
  if (missingTables.length === 0) {
    console.log(`✅ Todas as ${OFFLINE_TABLES.length} tabelas offline configuradas`);
  } else {
...
if (fs.existsSync(syncFile)) {
  ...
  const newTables = ['photoEvidence', 'interventionTypes', 'shiftConfigs', 'locationConfigs', 'notifications', 'qrCodes', 'tasks', 'flightSheets'];
  let missingInSync = [];
  ...
  if (missingInSync.length === 0) {
    ...
    errors.push(`❌ Tabelas faltando na sincronização: ${missingInSync.join(', ')}`);
  }
} else {
...
if (fs.existsSync(capacitorFile)) {
  ...
  if (missingPlugins.length === 0) {
    ...
    warnings.push(`⚠️ Plugins podem estar faltando: ${missingPlugins.join(', ')}`);
  }
} else {
...
if (fs.existsSync(packageFile)) {
  try {
    ...
    if (missingDeps.length === 0) {
      ...
      warnings.push(`⚠️ Dependências podem estar faltando: ${missingDeps.join(', ')}`);
    }
...
if (fs.existsSync(androidDir)) {
  ...
  warnings.push('⚠️ Projeto Android não encontrado (executar: npx cap add android)');
}
