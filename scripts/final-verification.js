...
const loadEnv = async () => {
  try {
    ...
    console.log(`${colors.yellow}⚠️  Could not load .env.production, using default values${colors.reset}`);
    return {};
  }
};
...
class AirPlusVerification {
  ...
  async verifyRLSPolicies() {
    ...
    await this.runTest('rls', 'RLS enabled on user_profiles table', async () => {
      const { data, error } = await this.supabase.rpc('check_rls_enabled', { table_name: 'user_profiles' });
    ...
    await this.runTest('rls', 'RLS helper functions exist', async () => {
      const functions = ['auth.user_has_role', 'auth.user_has_min_role_level', 'auth.user_has_permission'];
      
      for (const func of functions) {
        const { error } = await this.supabase.rpc('check_function_exists', { function_name: func });
        if (error) throw new Error(`Function ${func} does not exist: ${error.message}`);
      }
    });

    // Test 3: Verify policies exist for main tables
    await this.runTest('rls', 'Critical table policies exist', async () => {
      const tables = ['user_profiles', 'cleaning_forms', 'photo_evidence', 'aircraft', 'employees'];
      ...
      for (const table of tables) {
        ...
        if (error) throw new Error(`Could not check policies for ${table}: ${error.message}`);
    ...
    await this.runTest('rls', 'Role-based access control', async () => {
      ...
      if (error) throw new Error(`Roles table not accessible: ${error.message}`);
      if (!data || data.length === 0) {
        log.warning('No roles found in database - this may be expected for fresh install');
      }
    });
  }
  ...
  async verifyPhotoFunctionality() {
    ...
    await this.runTest('photos', 'Photo evidence table accessible', async () => {
      const { data, error } = await this.supabase
        .from('photo_evidence')
        .select('id')
        .limit(1);
      ...
      if (error && !error.message.includes('0 rows')) {
        throw new Error(`Photo evidence table not accessible: ${error.message}`);
      }
    });
    ...
    await this.runTest('photos', 'Storage buckets configured', async () => {
      ...
      for (const bucket of requiredBuckets) {
        if (!existingBuckets.includes(bucket)) {
          log.warning(`Bucket '${bucket}' not found - this may need to be created manually`);
        }
      }
    });
    ...
    await this.runTest('photos', 'Photo upload service available', async () => {
      ...
      if (!canvas) {
        log.warning('Cannot test photo upload in Node.js environment - requires browser');
        return;
      }
  ...
  async verifyPDFGeneration() {
    ...
    await this.runTest('pdf', 'Cleaning forms table structure', async () => {
      ...
      if (error && !error.message.includes('0 rows')) {
        throw new Error(`Cleaning forms table structure invalid: ${error.message}`);
      }
    });
    ...
    await this.runTest('pdf', 'PDF storage bucket available', async () => {
      ...
      if (error) {
        log.warning(`PDF storage bucket 'documents' may not exist: ${error.message}`);
      }
    });
    ...
    await this.runTest('qr', 'QR code URL generation', async () => {
      ...
      try {
        ...
        if (error && !error.message.includes('Object not found')) {
          log.warning(`Signed URL generation may have issues: ${error.message}`);
        }
      } catch (error) {
        log.warning(`QR URL generation test could not complete: ${error.message}`);
      }
    });
  ...
  generateReport() {
    ...
    console.log(`${colors.bright}Total Tests: ${totalPassed + totalFailed}${colors.reset}`);
    ...
    if (totalFailed === 0) {
      log.success('🎉 ALL VERIFICATIONS PASSED! System is ready for production.');
    } else if (successRate >= 80) {
      log.warning('⚠️  Most verifications passed. Review failed tests before production deployment.');
    } else {
      log.error('🚨 Multiple verifications failed. System needs attention before production deployment.');
    }
...
async function main() {
  ...
  console.log('╔══════���═══════════════════════════════════════════════════════╗');
  console.log('║                    AirPlus Aviation                         ║');
  console.log('║               Final System Verification                     ║');
  console.log('║                                                              ║');
  console.log('║    Verifying: RLS Policies • Photos • PDF • QR Codes        ║');
  console.log('╚══════════════════════════════════════════════════════════════╝');
  console.log(colors.reset);

  // Load environment
  const env = await loadEnv();
  const supabaseUrl = env.VITE_SUPABASE_URL || 'https://fyngvoojdfjexbzasgiz.supabase.co';
